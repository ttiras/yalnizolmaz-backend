...
class NhostMCPServer {
  ...
  private config: any;
  private clients: Map<string, NhostGraphQLClient> = new Map();
  ...
  private getClient(subdomain?: string): NhostGraphQLClient {
    const projects = this.config.projects || [];
    ...
    if (!this.clients.has(targetSubdomain)) {
      const project = projects.find((p: any) => p.subdomain === targetSubdomain);
      if (!project) {
  ...
  private async handleGraphQLQuery(args: any) {
  ...
  private async handleGraphQLMutation(args: any) {
